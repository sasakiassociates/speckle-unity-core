	...
	public class SpeckleStream : ScriptableObject
	{
		...
		public async UniTask<bool> TrySetNew(string streamId, string user, string server)
		{
			...
			try
			{
				...
				if (!branchName.Valid()) branchName = stream.branches.items.FirstOrDefault().name;
			...
			{
				SpeckleConnector.Console.Log(e.Message);
			}
		...
		private bool Setup()
		{
			if (_wrapper?.IsValid == true)
			{
				SpeckleConnector.Console.Log("Invalid input for stream");
				return false;
			}
		...
		public string GetUrl(bool isPreview)
		{
			...
			switch (Type)
			{
				case StreamWrapperType.Undefined:
					SpeckleConnector.Console.Warn($"{name} is not a valid stream, bailing on the preview thing");
					url = null;
					break;
		...
		public async UniTask<Texture2D> GetPreview()
		{
			...
			{
				SpeckleConnector.Console.Warn(www.error);
				return null;
			}
